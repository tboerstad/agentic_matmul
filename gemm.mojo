from amx import amx_bf16_gemm, amx_bf16_usable
...
@always_inline
def _compute_dtype[dtype: DType]() -> DType:
    ...
    store. The ceiling becomes the f32 FMA peak. SOL.md idea 2."""
...
struct RegisterTile[dtype: DType, MR: Int, NR_VECS: Int, NELTS: Int](
    ...
    `dtype` is the COMPUTE dtype (see `_compute_dtype`). The C block in memory
...
def _partial_n_microkernel[
    ...
    `_full_microkernel` (see there for the A addressing), masking only the C
    ...
    register-tile throughput. `_masked_microkernel` used to take these tiles
...
def _packed_gemm[
    ...
    All arithmetic runs in the compute dtype (`_compute_dtype`): the packed
    ...
    comptime CDT = _compute_dtype[dtype]()
...
def _prefill[
    ...
    comptime NELTS = simd_width_of[_compute_dtype[dtype]()]()
...
def _nopack_tail_row[
    ...
    for the same closure-capture reason as `_decode_fma_chunk`."""
...
def _l2_resident_kc[dtype: DType](tile_n: Int, k: Int) -> Int:
    ...
    matrix is bf16."""
    comptime elem = size_of[Scalar[_compute_dtype[dtype]()]]()
    var cap = 16384 // elem
...
def _box_fits_l2[dtype: DType](m: Int, n: Int, k: Int) -> Bool:
    ...
    epochs)."""
    var b_bytes = k * n * size_of[Scalar[_compute_dtype[dtype]()]]()
    return (
        m >= 64
        and m >= n
    ...
    def worker(worker_id: Int) {read c_view, read a_view, read b_view, mut bp_buf, read m, read n, read k, read num_cols, read num_i_panels, read num_workers, read total_tiles}:
        ...
        for u in range(start, end):
            ...
            if rows == MR and cols == NR:
                ...
                _partial_n_microkernel[dtype, MR, NR_VECS, NELTS, NR, KU](
                    c_view.sub(i0, jr), a_view.addr(i0, 0), 1, k,
...
def matmul_dispatch[
    ...
    the per-branch rationale.
        ...
        bf16 + AMX, M%32=N%16=K%32=0  amx_bf16_gemm  tdpbf16ps tile kernel
    ...
    # to the tile geometry it handles (whole 32-row M blocks, 16-column C
    # tiles, 32-deep K pair-steps) and to above the tiny cutoff. Every other
    # dtype, machine, and shape falls through to the cascade unchanged.
